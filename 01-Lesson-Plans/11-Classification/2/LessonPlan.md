...
Point out to students that whenever we create dummy variables, we need to be mindful of the so called 'dummy variable trap'. Take a moment to briefly introduce the intuition behind this. Using the example with male and female above, one of them is redundant since we know that if a row has a 1 in gender_female, then gender_male has to be 0 (explain to students that for simplicity, we are assuming individuals can only be either one of the categories, but this example would also apply if we allowed for more gender categories).

Why is this a problem? If 2 variables are perfectly correlated, our algorithm can't tell apart which one of the variables caused the change in the dependent variable. You can use the 2 examples below to illustrate this problem:
...
We can circumvent this problem by dropping one of the variables, making that the reference or base case, and then estimating the effect when the other variables turn 1.
...
> Note: The Decision Tree model makes slightly different predictions every time it is run, because of random fluctuations in how the trees are constructed. Your output may differ slightly from what you see above.

* After observing the results, it can be seen that the model's accuracy (`0.544`) is low. Also, precision and recall are not good enough to state that the model will be good at predicting fraudulent loan applications.
