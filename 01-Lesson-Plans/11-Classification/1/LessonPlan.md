...
* The slides for this lesson can be viewed on Google Drive here: [Lesson Slides](https://docs.google.com/presentation/d/1OWPhlLMHuinFyYb6jtTYsYoij206tRC2xROhSzBnjRQ/edit?usp=sharing).
...
* Communicate that Logistic Regression is a statistical method for predicting binary outcomes from data. With linear regression, our linear model may provide a numerical output&mdash;such as a credit score, ranging from 0 to 850. With logistic regression, that numerical value for credit score could be translated to a probability between 0 and 1. This discrete output could then be labeled as “good credit” (a label of `1`) vs. “bad credit” (a label of `0`). Various data could be used to predict which label a borrower is, based on their number of continuous on-time payments, number of accounts, credit age, or other factors thought to predict credit quality.

* With this example (shown in the image below), these predictions, based on one or more useful variables like payment history, get converted into a probability of being “good credit” or not.

* For example, if the probability that a borrower is a “good credit” borrower exceeds 50% (0.50), the model classifies the borrower as such.

* The range of probabilities, produced from these variables used for prediction, is shown by the black “s-shaped” curve in the image below, otherwise known as the prediction curve. Given information on any borrower, we can now make a prodiction based on where their data sits upon this curve.

  ![logistic_1.png](Images/logistic_1.png)
  ![logistic_2.png](Images/logistic_2.png)

* How do we translate continuous data like payment history into a probability of “good credit” ranging from 0 to 1? A mathematical transformation known as a sigmoid function does this conversion for us.

* A sigmoid function is used "behind the scenes" in any logistic regression model in order to transform our model’s predictions about the borrower—which may rely on many more variables than just payment history—into a single probability value ranging from 0 (0% chance of good credit) to 1 (100% chance of good credit).

  * In short, it's a function that you can put in any range of number, and the output will be a number which ranges only from `0` to `1` (i.e., the same range as a probability).
