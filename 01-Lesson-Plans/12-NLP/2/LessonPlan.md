...
* **NOTE:** The news changes constantly and the api is frequently refreshing. As such, the above parameters might not return as many results as the example in the lesson plan.  If **no** results are returned, try setting the `q=` parameter to another news-worthy term, or removing the `country=` parameter to broaden the search.
