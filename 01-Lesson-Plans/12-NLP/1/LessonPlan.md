...
* The next lesson will utilize the [News API](https://newsapi.org/), a free API for developers that retrieves metadata for headlines and news. Though an in-class activity will guide students through the News API setup process, it would be helpful to slack out the link prior to the next class for those students that would like to prepare early. 
