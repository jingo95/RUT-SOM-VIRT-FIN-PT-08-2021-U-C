...
* **Important:** The [Deep Learning Installation Guide Folder](../../14-Deep-Learning/Supplemental/Deep_Learning_Installation_Guide) for installing Tensorflow should be slacked out prior to today's class, as students need to run these installations before class to conserve time for lessons and activities. 

* **Important:** Students with Apple Silicon M1 computers will not be able to install Tensorflow using traditional methods. The simplest solution is to have these students run their notebooks in [Google Colab](https://colab.research.google.com/) which comes with support for TensorFlow. Notebook files for the first two activities will run in Google Colab without alteration, however an alternate "M1" specific file is provided for all other activities.

* **Important:** The activity titled _Instructor Do: Preparing Data for Neural Networks_ ([04-Ins_Preparing_Data](Activities/04-Ins_Preparing_Data)) **will not run on Apple Silcon M1 computers**, despite correct installation of Tensorflow using the methods in the installation guide. Apple Silicon M1 users **must** run this activity by uploading the file into [Google Colab](https://colab.research.google.com/).
...
The slides for this lesson can be viewed on Google Drive here: [Lesson Slides](https://docs.google.com/presentation/d/1LmNtLghBoQeoonYzjKbSg40meJveGEOrwm2hqRDfWSg/edit?usp=sharing).
...
* Using an activation function, we can normalize the output to a range between -1 and 1, or 0 and 1, the latter being a representation of a probability. 

* The type of output range we obtain depends on the activiation function. To obtain probabilities, we need to choose activation functions such as Softmax which provide a range between 0 and 1. 

* Instead of a _yes/no_ decision, with an activation function, we get the probability of yes, similar to using  a logistic regression.
...
Ask students if they have already installed TensorFlow and, if some students have not done so, slack out the [Deep Learning Installation Guide](../../14-Deep-Learning/Supplemental/Deep_Learning_Installation_Guide/deep_learning_installation_guide.md) and have your TAs assist students in the process while you continue to the demo. 
...
* The next step is to add the first layer of our neural network using the `add()` method and the `Dense()` class (note: the graph below is for illustrative purposes and the number of input nodes does not match the number of inputs in the code)
...
* Finally, some predictions are made using new dummy sample data and the `predict` method in conjunction with a threshold of `0.5`.  Anything under `0.5` is classified as `0` and anything over `0.5` is classified as `1`.  To do this we use the following code: 
...
**Important:** This activity **will not run on Apple Silcon M1 computers**, despite correct installation of Tensorflow using the methods in the installation guide. Apple Silicon M1 users will have to run this activity by uploading the file into [Google Colab](https://colab.research.google.com/).
