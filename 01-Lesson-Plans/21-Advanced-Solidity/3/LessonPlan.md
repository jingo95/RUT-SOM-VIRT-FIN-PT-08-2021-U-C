...
* Some of todays activities will build off the same files. Remix does not allow opening multiple files with the same name so those files are named uniquely to ease import into Remix. Instructor files will have the extension `_ins_v1` and student files will have the extension `_stu_v1` with version numbers increasing as the activity numbers increase.
...
[ArcadeTokenMintable_ins_v1.sol](Activities/02-Ins_Preparing_a_token_for_a_Crowdsale/Solved/ArcadeTokenMintable_ins_v1.sol)

Open [Remix](https://remix.ethereum.org), import the file named `ArcadeTokenMintable_ins_v1.sol`, and start with the following boilerplate:
...
Once again, expand upon the concept of inheritance by opening up the `ERC20Mintable.sol` in Remix from the sidebar (you will need to save `ArcadeTokenMintable_ins_v1.sol` to get Remix to load the library):
...
Close OpenZeppelin's `ERC20Mintable` and navigate back to `ArcadeTokenMintable_ins_v1.sol`.
...
* [ArcadeTokenMintable_stu_v1.sol](Activities/03-Stu_ERC20Mintable_Token_Design/Unsolved/ArcadeTokenMintable_stu_v1.sol)

### 7. Instructor Do: Review ERC20 Mintable Token Design (10 min)

**File:**

* [ArcadeTokenMintable_stu_v1.sol](Activities/03-Stu_ERC20Mintable_Token_Design/Solved/ArcadeTokenMintable_stu_v1.sol)
...
* [ArcadeTokenMintable_ins_v2.sol](Activities/04-Ins_Crowdsales/Solved/ArcadeTokenMintable_ins_v2.sol)

* [ArcadeTokenSale_ins_v1.sol](Activities/04-Ins_Crowdsales/Solved/ArcadeTokenSale_ins_v1.sol)
...
Navigate to [Remix](https://remix.ethereum.org) and open the starter file: [ArcadeTokenSale_ins_v1.sol](Activities/04-Ins_Crowdsales/Solved/ArcadeTokenSale_ins_v1.sol).
...
Your contract should now match the [ArcadeTokenSale_ins_v1.sol](Activities/04-Ins_Crowdsales/Solved/ArcadeTokenSale_ins_v1.sol) solution linked above.

Navigate back to the `ArcadeTokenMintable_ins_v.sol`.
...
* [ArcadeTokenMintable_stu_v2.sol](Activities/05-Stu_ArcadeTokenSale/Unsolved/ArcadeTokenMintable_stu_v2.sol)

* [ArcadeTokenSale_stu_v1.sol](Activities/05-Stu_ArcadeTokenSale/Unsolved/ArcadeTokenSale_stu_v1.sol)
...
* [ArcadeTokenMintable_stu_v2.sol](Activities/05-Stu_ArcadeTokenSale/Solved/ArcadeTokenMintable_stu_v2.sol)

* [ArcadeTokenSale_stu_v1.sol](Activities/05-Stu_ArcadeTokenSale/Solved/ArcadeTokenSale_stu_v1.sol)
...
* This function sends the amount of `balance[msg.sender]`s ether to `msg.sender`.
...
* Notice that because call.value is returning a tuple,  we first save the boolean return value and then check whether it is true or false.

Inside the `widthdrawBalance` function define a new `uint` named `amount` and use it to save the current `msg.sender`'s `balance`. Then set the `balance` to 0.
...
* We are going to define a new `uint` named `amount` and use it to save the current `msg.sender`'s `balance`.

* Then we are going to set the `balance` to 0.
