...
* The  standard deviation exceeds the RMSE, indicating that the model may be useful. In other words, on average, there are smaller swings in the error than the recorded salaries.
...
Explain that we need a column of returns, and lagged returns, which will be regressed:
  ...
  * Here, unlike previous examples with two separate variables, `Return` values are regressed against `Lagged_Return` values, which is also called an autoregression.
...
### 16. Instructor Do: The Career Team (35 min)
...
* All students should submit their completed resume to a Career Material Advisor on BCS by the end of class. If students do not complete their resume on time, tell them to submit their resume to BCS by the end of the week. This allows students to get feedback on the work they have done. Career Material Advisors should respond to students within 96 hours with feedback on their resume.
