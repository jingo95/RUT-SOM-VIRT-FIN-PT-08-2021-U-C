...
* Slack out the [`HD-Wallet-Derive` Install Guide](../Supplemental/HD_Wallet_Derive_Install_Guide.md) and make sure students have it installed prior to next class so that installation troubleshooting can be conducted during office hours prior to using the tool in class on day three of the unit. 

* If errors occur with the Geth class activities, it is likely because the incorrect version of Geth has been installed. Students should **not** install the most recent version of Go Ethereum Tools. The correct version to install is **"Geth & Tools 1.9.7"**. 
...
* **NOTE:** Some instructors have reported that this code results in the following error: `AttributeError: function/symbol 'SHA256_init' not found in library`. To resolve this issue, run the following in your terminal: `pip uninstall pycryptodome` followed by `pip install pycryptodome==3.9.8`.
