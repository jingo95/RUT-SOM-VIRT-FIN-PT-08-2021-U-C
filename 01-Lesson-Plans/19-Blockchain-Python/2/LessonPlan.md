...
* Remind students to install the `hd-wallet-derive` tool if they have not already by slacking out the [`HD-Wallet-Derive` Install Guide](../Supplemental/HD_Wallet_Derive_Install_Guide.md) again. Urge those students encountering installation issues to attend office hours so this can be conducted prior to using the tool during class. 
